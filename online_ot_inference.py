...
def main(args):
    ...
    for k, human_corr_idx in enumerate(tqdm.tqdm(human_corr_indices, desc="OT matching between corresponding episodes")):
        ...
        expert_weight = torch.ones((demo_len // n_skip_frame,), device=device) / float(demo_len // n_skip_frame)
        expert_indices = torch.arange(demo_len // n_skip_frame, device=device)
        greedy_ot_plan = torch.zeros((demo_len // n_skip_frame, rollout_len // n_skip_frame), device=device)
        greedy_ot_cost = torch.zeros((rollout_len // n_skip_frame,), device=device)
        ...
        # Visualization
        cell_size = 1
        fig = plt.figure(figsize=(rollout_len // n_skip_frame * cell_size, demo_len // n_skip_frame * cell_size + 1))
        ...
        im = main_ax.imshow(greedy_ot_plan.detach().cpu().numpy(), cmap='viridis', aspect='auto')
        plt.colorbar(im, ax=main_ax, shrink=0.8)
        main_ax.set_xticks(np.arange(demo_len // n_skip_frame))
        main_ax.set_yticks(np.arange(rollout_len // n_skip_frame))
        main_ax.set_xticklabels([''] * (demo_len // n_skip_frame))
        main_ax.set_yticklabels([''] * (rollout_len // n_skip_frame))
        main_ax.tick_params(axis='both', which='both', length=0)

        for y in range(demo_len // n_skip_frame):
