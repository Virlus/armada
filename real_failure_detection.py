...
from util.failure_detection_util import cosine_distance, optimal_transport_plan, rematch_expert_episode
    ...
    try:
        while True: 
            ...
            while True:
                ...
                # # If failed to detect failure, lower the percentiles
                # if keyboard.finish and INTV in action_mode and len(failure_indices) == 0:
                #     print("No failure detected, lowering the action inconsistency percentile and OT percentile")
                #     action_inconsistency_percentile = max(action_inconsistency_percentile - 5, 0)
                #     ot_percentile = max(ot_percentile - 5, 0)
                #     expert_action_threshold = np.percentile(np.array(success_action_inconsistencies), action_inconsistency_percentile) if len(success_action_inconsistencies) > 0 else None
                #     expert_ot_threshold = np.percentile(success_ot_values, ot_percentile) if len(success_ot_values) > 0 else None
                #     print("Reset the expert OT threshold to ", expert_ot_threshold)
                #     print("Reset the expert soft OT threshold to ", expert_soft_ot_threshold)
