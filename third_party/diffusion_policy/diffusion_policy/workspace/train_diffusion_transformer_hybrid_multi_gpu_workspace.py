...
class TrainDiffusionTransformerHybridMultiGPUWorkspace(BaseWorkspace):
    ...
    def __init__(self, cfg: OmegaConf, rank, world_size, device_id, device='cuda:0', output_dir=None):
        ...
        # configure model
        self.model: DiffusionTransformerHybridImagePolicy = hydra.utils.instantiate(cfg.policy).to(device)
        ...
        cfg.optimizer.learning_rate = cfg.optimizer.learning_rate * world_size # math.sqrt(world_size)
        self.optimizer = self.model.module.get_optimizer(**cfg.optimizer)
    ...
    def run(self, rank, world_size, device_id, device):
        ...
        sampler_val = DistributedSampler(dataset=val_dataset, num_replicas=world_size, rank=rank, shuffle=True, seed=cfg.training.seed, drop_last=True)
        val_dataloader = DataLoader(val_dataset, sampler=sampler_val, **cfg.val_dataloader)
        ...
        lr_scheduler = get_scheduler(
            ...
            num_warmup_steps=cfg.training.lr_warmup_steps // world_size,
            num_training_steps=(
            ...
            for local_epoch_idx in range(cfg.training.num_epochs):
                ...
                if (self.epoch % cfg.training.sample_every) == 0:
                    with torch.no_grad():
                        ...
                        if cfg.task.rel_ee_pose:
                            start_timestep = cfg.n_obs_steps
                            pred_action = pred_action[:, start_timestep:]
                            gt_action = gt_action[:, start_timestep:]
...
def main(cfg):
    workspace = TrainDiffusionTransformerHybridMultiGPUWorkspace(cfg)
    workspace.run()
