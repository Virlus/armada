        ...
        while j < max_episode_length:
            ...
            for step in range(Ta):
                if step > 0:
                    start_time = time.time()
                # last_T = np.eye(4)
                # last_T[:3, :3] = last_r.as_matrix()
                # last_T[:3, 3] = last_p
                # rel_T = np.eye(4)
                # rel_T[:3, :3] = R.from_quat(action_seq[step, 3:7]).as_matrix()
                # rel_T[:3, 3] = action_seq[step, :3]
                # curr_T = np.matmul(last_T, rel_T)
                # curr_p = curr_T[:3, 3]
                # curr_r = R.from_matrix(curr_T[:3, :3])
